...
import { getAll, getAllForUser, getAllSubmitted } from '../domain/exercises.js'
...
export const getSubmittedExercises = async (req, res) => {
  const userid = Number(req.params.id)
  try {
    const exercises = await getAllSubmitted(userid)
    return sendDataResponse(res, 200, { exercises })
  } catch (e) {
    if (e instanceof Prisma.PrismaClientKnownRequestError) {
      if (e.code === 'P2003') {
